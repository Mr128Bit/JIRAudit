...
from ..marketplace_api import Plugin  # pylint: disable=unused-wildcard-import wildcard-import
...
class JiraAPI:  # pylint: disable=too-many-public-methods too-many-instance-attributes
    ...
    class API_ENDPOINTS(Enum):
        ...
        SERVICEDESKINFO = "/rest/servicedeskapi/info"
    ...
    def websudo_request(self, url: str) -> requests.models.Response:
        ...
        try:
            ...
            raise JiraRequestException(message=f"Request threw an exception {e}") from e
    ...
    def get_server_info_raw(self) -> dict:
        ...
        if self.authenticated:
            ...
            response = requests.get(self.BASE_URL + self.API_ENDPOINTS.SERVERINFO.value)
    ...
    def get_servicedesk_version(self): 

        try:
            url = f"{self.BASE_URL}{self.API_ENDPOINTS.SERVICEDESKINFO.value}"
            resp = requests.get(url=url, verify=False, headers=self.HEADERS, timeout=10)
            if resp.status_code == 200:
                response = resp.json()
                licensed = response.get("isLicensedForUse")
                return licensed
            else:
                raise JiraRequestException(
                    message=f"Request resulted in HTTP status {resp.status_code}"
    ...
    def is_servicedesk_licensed(self) -> bool:
    ...
    def is_servicedesk_installed(self) -> bool:
